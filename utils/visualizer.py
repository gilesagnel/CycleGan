...
class Visualizer():
    ...
    def __init__(self,opt):
        ...
        if self.display_id > 0:  
            import visdom
            self.vis = visdom.Visdom(server=opt.display_server, port=self.port, env=opt.display_env)
            if not self.vis.check_connection():
                self.create_visdom_connections()

        if self.use_wandb:
            self.wandb_run = wandb.init(project=self.wandb_project_name, name=self.name, config=None) if not wandb.run else wandb.run
    ...
    def display_current_results(self, visuals, epoch, save_result):
        ...
        if self.use_html and (save_result or not self.saved):  
            ...
            webpage = util.HTML(self.web_dir, 'Experiment name = %s' % self.name, refresh=1)
